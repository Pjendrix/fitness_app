  ...
  data(uid) {
    ...
    return {
      async loadAll() {
        ...
        return { templates: t.docs.map((d) => d.data()), workouts: w.docs.map((d) => d.data()), prs, library: ex.exists() ? ex.data() : null };
      },
...
const demoBackend = {
  ...
  data() {
    return {
      async loadAll() {
        const d = readLS();
        return { ...d, library: d.library || (d.exercises?.length ? { list: d.exercises } : null), workouts: [...d.workouts].sort((a, b) => b.startedAt - a.startedAt) };
      },

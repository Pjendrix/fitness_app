...
export default function Home({ go }) {
  ...
  return (
    <div className="screen">
      ...
      {active ? (
        <section className="glass glass-glow">
          <p className="muted">Rozdělaný trénink</p>
          <h2 className="big">{active.name}</h2>
          <button className="btn btn-primary btn-block" onClick={() => go('workout')}>Pokračovat</button>
        </section>
      ) : (
        <section className="glass glass-glow">
          <p className="muted">Rychlý start{group === next ? ' · na řadě' : ''}</p>
          ...
          <p className="muted small">{tpl.exercises.length} cvičení · {tpl.exercises.reduce((s, e) => s + e.sets, 0)} sérií</p>
          <button className="btn btn-primary btn-block" onClick={start}>Začít {tpl.name}</button>
        </section>
      )}

      <section className="stats">
        <div className="glass stat"><span className="num">{week.count}</span><span className="muted small">tréninků tento týden</span></div>
        <div className="glass stat"><span className="num">{week.volume ? fmtNum(Math.round(week.volume / 100) / 10) : 0}<small> t</small></span><span className="muted small">objem tento týden</span></div>
        <div className="glass stat"><span className="num">{workouts.length}</span><span className="muted small">tréninků celkem</span></div>
      </section>

      <section>
        <h3 className="section-title">Nedávné rekordy</h3>
        {recentPrs.length ? (
